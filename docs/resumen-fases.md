...
- **Integración Transaccional e Idempotencia (view.php)**:
  - Para evitar la creación de salas duplicadas en Matrix si un usuario hace "doble clic" o lanza múltiples peticiones asíncronas, se utilizó la **Locking API** nativa de Moodle (corelocklock_config).
...
- **Evitación de Conflictos (Doxygen vs Moodle Autoloader)**:
  - **El Problema**: Doxygen fallaba (modo estricto) al encontrar nombres de clases con guión bajo precedidos por namespaces PHP (block_bdcmapeo_client). Si se retiraban los namespaces pero los ficheros se dejaban en la carpeta classes/, el autoloader estricto de Moodle 4+ provocaba un error fatal (exit 1) durante el proceso de actualización de la base de datos de Docker.
...
- **Aprovisionamiento Automático desde Moodle**: Se integró en la API Mapeo el endpoint POST /cursos. El bloque de Moodle (block_bdc) ahora registra un observador de eventos (coreeventcourse_created) que intercepta la creación de cursos en la plataforma.
...
- **Refactorización de Enlaces Doxygen**: Se corrigieron las etiquetas de las subpáginas en arquitectura.md y mapeo-alumno-repositorio.md (subpage decisiones_tecnicas y mapeo_alumno_repo) para garantizar una correcta generación del árbol de documentación de Doxygen sin advertencias (WARN_AS_ERROR=YES).

---

## Fase 10: Dashboard de Trazabilidad y Métricas (BDC Metrics)
Esta fase introdujo un panel de control independiente (Frontend React + Backend FastAPI) destinado a los profesores para visualizar la telemetría, el nivel de interacción y el progreso de los alumnos a partir de los datos recopilados por el bot.

### Desarrollo e Implementación
- **Autenticación Delegada y Resiliencia**: Se integró el inicio de sesión contra los Web Services de Moodle. Para soportar entornos de desarrollo aislados, la API (metrics-api) implementa un fallback que asume como válidas las credenciales si Moodle responde con errores de infraestructura (ej. servicenotavailable), rechazando únicamente el código explícito de invalidlogin.
- **Persistencia de Sesión JWT**: El frontend de React implementó la preservación del token JWT en localStorage, garantizando que la sesión del profesor sobreviva a las recargas del navegador e hidratando automáticamente el contexto de autenticación al arrancar.
- **Separación Lógica de Roles (RBAC)**: 
  - La interfaz de navegación (Layout.tsx) condiciona su renderizado según el flag is_teacher inyectado en el token JWT.
  - Se habilitó un **Perfil de Profesor** exclusivo que aglutina los cursos asignados al docente, junto con un RoleGuard estricto en el backend que deniega con HTTP 403 Forbidden intentos de acceso no autorizados a métricas de estudiantes.
- **Mejoras UX en Dashboard de Curso y Perfiles Individuales**:
  - Inyección dinámica de metadatos de enrutamiento: Al navegar al perfil individual de un estudiante, su nombre y la asignatura se transmiten por el estado interno del enrutador de React (useLocation). Esto permite mostrar títulos descriptivos (ej. *Perfil de alumno1 (Ecuaciones Diferenciales II)*) en lugar de IDs genéricos de base de datos.
  - Se solucionó una omisión crítica que causaba errores 401 Unauthorized al visitar perfiles individuales añadiendo la inyección explícita de cabeceras Authorization en las llamadas fetch anidadas.
  - Se desarrolló un sistema de ordenación intuitivo con selectores desplegables en la tabla de alumnos matriculados, permitiendo clasificar visualmente a los alumnos por nombre, número de interacciones o última actividad.
- **Consolidación Integral de Interacciones (Chat + Archivos)**:
  - Se corrigió el servicio recolector de métricas (metrics-worker/worker.py) para contabilizar de forma global tanto los mensajes de chat puros (INTERACTION) como las ingestas documentales (INGEST).
  - Las subidas de archivos en las salas Matrix a través del bot ahora se catalogan como interacciones de tipo file_upload, ofreciendo a los profesores una visión desagregada de cómo los estudiantes enriquecen la Base de Conocimiento de sus repositorios.
