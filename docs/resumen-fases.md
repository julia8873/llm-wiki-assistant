...
## Fase 11: Extracción y Almacenamiento de Interacciones (OKF)
Esta fase implementó el mecanismo unificado para interceptar la respuesta del modelo hacia el alumno y estructurar simultáneamente los metadatos de la interacción sin duplicar el coste de tokens del LLM, depositándolos de manera segura en el repositorio del alumno siguiendo el estándar OKF.

### Desarrollo e Implementación
- **LLM Unificado (Cero Coste Adicional)**: Se modificó la llamada principal al LLM en Maubot para exigir (via System Prompt y `response_format`) un output JSON (`{"respuesta_alumno": "...", "tipo_interaccion": "...", "concepto": "..."}`). Con esto, la respuesta natural y los metadatos de clasificación se generan en un solo pase.
- **Batching en Worker Asíncrono**: Para evitar el encolado infinito y la sobreescritura de commits por concurrencia o límites de rate (API abuse), se introdujo lógica de **batching por tiempo** configurada globalmente mediante `batching_interacciones_sec` en `config.yaml`. Esto asegura que los eventos OKF se consoliden en el repositorio del alumno.
- **Bloqueo Distribuido (Redis)**: Se empleó la función `distributed_repo_lock` para proteger el directorio local durante el git pull/push, previniendo corrupción al existir múltiples workers intentando commitear al mismo tiempo (ej. `sync-worker-1` y `sync-worker-2`).
- **Endpoints de Metadatos (Mapeo API)**: Se introdujeron endpoints HTTP (con soporte de paginación) para recuperar las interacciones almacenadas, posibilitando su análisis sin tener que clonar repositorios del lado del cliente o frontend.
- **Protección de Datos (PII Redaction)**: Se estableció un pipeline rígido de saneamiento antes de insertar datos al OKF, impidiendo la filtración de nombres reales de Moodle en la estructura JSONL, utilizando como alias el seudónimo Matrix.

### Lecciones y Hallazgos Críticos
- **Falsos Positivos Silenciosos (Excepciones en Producción)**: Durante la verificación de la lectura de `config.yaml` desde el entorno del `sync-worker`, se detectó un fallo crítico en la arquitectura de logging. La dependencia `pyyaml` no existía en el contenedor, lo que producía un ModuleNotFoundError. Debido a un bloque `except Exception` que servía de "fallback seguro" (asignando el valor por defecto de 300 segundos), la falta de dependencia se enmascaraba en un error silencioso, eludiendo todos los tests de E2E.
- Esto obligó a endurecer la validación:
  1. Se añadió `pyyaml` explícitamente al `Dockerfile` del entorno base de `maubot`.
  2. Quedó documentada la necesidad de monitorizar activamente los canales de nivel `logger.warning`, pues fallos estructurales o de infraestructura (volúmenes de Docker no montados, dependencias perdidas) pueden manifestarse únicamente como un comportamiento "por defecto" si no se inspeccionan rigurosamente.
- **Doxygen y Configuración Estricta**: La exigencia de calidad en la generación de documentación (`WARN_AS_ERROR=YES`) reveló la importancia de mantener enlaces y sintaxis markdown escrupulosamente limpios en archivos como `README.md`, pues los enlaces rotos generaban paradas (`TK_EOF`) en el pipeline de documentación en integración continua.
- **Pérdida de Datos por Batching en Entorno Temporal Git**: En producción se descubrió un fallo arquitectónico severo. El batching temporal acumulaba interacciones en el árbol de trabajo (working tree) del directorio temporal `/tmp/llm_wiki_repos`. Sin embargo, cada ejecución de *task* en el worker invocaba `asegurar_repo_local()`, el cual hacía un `git reset --hard origin/HEAD` para limpiar el entorno. **Esto borraba irremediablemente todas las interacciones de los últimos 5 minutos que no hubieran hecho commit.** Como medida paliativa inmediata, se retiró la lógica de batching de `tasks.py` para sincronizar en tiempo real (commit y push inmediato por interacción). Se endureció además `git_utils.py` para abortar tareas (lanzando `RuntimeError`) si `git fetch` falla por red, asegurando la idempotencia y confiando en los reintentos automáticos de `RQ`. **(Pendiente de rediseño futuro: usar un fichero `untracked` temporal como buffer de batching para evitar el borrado por `git reset`)**.

---

## Fase 11.1: Corrección de Fuente Única de Verdad (Interacciones)
Esta fase subsanó las asincronías y dependencias circulares derivadas de la Fase 11, unificando definitivamente el sistema de registro (logging) y la persistencia de interacciones del LLM.

### Desarrollo e Implementación
- **Consolidación de Flujo de Escritura**: Se eliminó la tarea redundante `log_interaccion_extraccion_task` y se unificó todo el proceso de registro bajo `log_interaction_task`. Ahora, al generarse una respuesta del LLM, el bot (Maubot) inyecta directamente los metadatos generados (`tipo_interaccion` y `concepto`) en el payload enviado al worker, garantizando la unicidad transaccional.
- **Idempotencia y Fallback Offline**: El archivo `logs/interacciones/YYYY-MM-DD.jsonl` del repositorio del alumno actúa como la **única fuente de verdad**. Si falla la subida a GitHub (push), la tarea es reencolada por `RQ`. Para prevenir registros duplicados, el worker compara el hash (SHA-256) de la nueva línea con el archivo local, omitiendo la escritura en caso de duplicidad.
- **Migración del Frontend (Capa 2)**: Se refactorizó la API de métricas (`bdc-trazabilidad`) para que deje de buscar el contenido de los mensajes en la base de datos local (PostgreSQL). El endpoint `/interacciones/{id}/contenido` ahora recupera el archivo original directamente desde la API Contents de GitHub en tiempo real, emparejando el ID (hash) de la interacción y decodificando el Base64, evitando las demoras impuestas por la CDN estática (`raw.githubusercontent`).
- **Seguridad en Crudo**: Aunque el backend censura los identificadores personales (PII) antes de devolver los mensajes a la UI, los *prompts* originales sin censurar quedan inmutables y registrados en los repositorios privados de los alumnos en GitHub, garantizando la pureza para auditoría, aunque imponiendo exigencias futuras de RBAC y privacidad en la plataforma GitHub.
- **Limitación de Tasa (Rate-Limit) y Escalamiento**: La decisión de mantener un commit-por-interacción (sin batching) asume un bajo volumen de mensajes. Si el proyecto escala a un aula con muchos alumnos simultáneos, el presupuesto de rate-limit de la API de GitHub podría agotarse. **El commit-por-interacción es el primer sitio a revisar en caso de problemas de escalamiento, revaluando la introducción de batching seguro.**

---

## Fase 13: Hardening y Estabilización (Planificada)
- **Eliminar Compatibilidad Histórica**: Eliminar el bloque de compatibilidad temporal en `get_interacciones_metadatos` (bdc-trazabilidad) diseñado para el formato JSONL pre-11.1 (dos líneas por interacción). Esto debe hacerse una vez que se confirme mediante pruebas automatizadas que ya no quedan archivos JSONL con el formato antiguo en ningún repositorio de alumno activo.
