...
|---|---|---|
| `.env.example` | `.env` | Variables globales Docker (`OPENAI_API_KEY`, DB Password y otros secretos). El PAT de GitHub se genera a partir de `config/config.yaml` y no debe introducirse manualmente en los `.env.example`. |
